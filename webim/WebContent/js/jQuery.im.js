...
//                        });
                        ...
                        $.ajax({url:"ajax/im_window.html",dataType:"html",success:function(data){
                            if(options.defaultWindow!=""){
                            	$(".im_contact").live("click",function(){
                                    //选中联系人
                                    ...
                                    var uid=$(this).attr("uid");
                                    $.getJSON("servlet/ChatServlet?sAction=userinfo&uid="+uid,function(json){
                                        //打开新窗口
                                        openWindow(options.defaultWindow,json[0].id,json[0].isOnline,json[0].UserAvatar,json[0].cname,json[0].UserName,data);
                                    });
                                    //清除消息提示
                    ...
                    function Friends(uid){
                        ...
                        $.getJSON("servlet/ChatServlet?sAction=friends&uid="+uid,function(json){
                            for(var i=0;i<json.length;i++){
                            	if(json[i].type=="0"){
                            		$("#imRencentContacts").append('<div class="im_contact online" uid="'+json[i].id+'"><i><img src="'+json[i].UserAvatar+'" width="30" height="30" /><sup></sup></i><span style="cursor: pointer;">'+json[i].UserName+'</span><a href="#" target="_blank"></a></div>');
                                    if(json[i].m!=0){
                                       $("#imRencentContacts .im_contact:eq("+i+")").find("span").append("<b></b>");
                                    }
                            	}else if(json[i].type=="1"){
                            		$("#imFriends").append('<div class="im_contact online" uid="'+json[i].id+'"><i><img src="'+json[i].UserAvatar+'" width="30" height="30" /><sup></sup></i><span style="cursor: pointer;">'+json[i].UserName+'</span><a href="#" target="_blank"></a></div>');
                            		if(json[i].m!=0){
                            ...
                            Scroll();
                            //$("#imRencentContacts").prev("h4").find("i").html("(5/"+json[i].length+")");
                            //$("#imFriends").prev("h4").find("i").html("(3/"+json[i].length+")");
                            if(5!=0&&3!=0){
                               newMsg();//新消息提醒
                            }
                        });
                    ...
                    function openWindow(cid,uid,o,a,n,c,m){
                        ...
                        //操作当前聊天窗口
                        operatewindow(cid,uid);
                        //加载聊天记录
                        msgList(cid,uid);
                    }
                    /****加载聊天记录****/
                    function msgList(cid,uid){
                    	$(".im_chats .im_chat_content:last").after("<div id='im_chat_content'></div>");
                        ...
                        $.getJSON("servlet/ChatServlet?sAction=chats&cid="+cid+"&uid="+uid,function(json){
                            ...
                            $.each(json,function(i){
                                if(json[i].pid==uid){
                                	msgList+='<div class="im_chat_content"><i>'+json[i].D+'</i><div class="im_chat_txt">'+json[i].M+'</div></div>';
                                }else if(json[i].pid==cid){
                                	msgList+='<div class="im_chat_content myself"><i>我：'+json[i].D+'</i><div class="im_chat_txt">'+json[i].M+'</div></div>';
                                }
                            });
                            
                            
                            //$(".im_chats .im_chat_content:last").nextAll().remove();
                            //$(".im_chat_content div").remove(); 
                            //$(".im_chat_content myself div").remove();
                            $("#im_chat_content").html("");
                            $("#im_chat_content").html(msgList);
                        	//$(".im_chats .im_chat_content:last").after(msgList);
                    ...
                    /****聊天窗口选项****/
                    function operatewindow(cid,uid){
                        //表情面板
                        ...
                        $("#imcontact").click(function(){
                            if($(this).hasClass("im_addcontact")){
                                art.confirm('您确定要将对方添加为好友吗？', function () {
                                	 $.post('servlet/ChatServlet?sAction=addFriend&cid='+cid+'&uid='+uid, function(data) {
                                     });
                                    //提交数据并刷新联系人列表
                                    Friends(options.defaultWindow);
                    ...
                    function sendMsg(cid,uid){
                        ...
                        if(msgContent.html().length!=0&&msgContent.html().length<options.msgMaxSize){
                            ...
                            urlFilter();
                            //此处提交用户消息至服务器
                            $.post('servlet/ChatServlet?sAction=sendMsg&cid='+cid+'&uid='+uid+'&msgContent='+encodeURI(encodeURI(msgContent.html())), function(data) {
                            });
